        ...
        while (this.shutdown == false)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // schema size is the seek cost of the initColumnOrder,
                    // note that initColumnOrder is the previous order of the table (if there exist one) or
                    // the default column order determined by the order of COL_ID in COLS in the metadata schema
                    // if the table does not have any existing layout.
                    ...
                    // ordered seek cost is the seek cost of the 'optimized' column order generated in this round
                    // of optimization. And the optimized column ordered will be saved as LAYOUTS.LAYOUT_ORDER of
                    // this round.
                    ...
                    // when creating the ExecutorContainer instance, scoaPixels.setup() is called.
                    // after calling setup(), scoaPixels.getCurrentWorkloadSeekCost() returns the
                    // of the current workload and columnlet order. Note that the returned seek cost
                    // may be much higher that the ordered seek cost or the schema seek cost printed
                    // out above. Because we rebuild pseudo workload and column(let) order for scoaPixels.

                    // if scoaPixels.getCurrentWorkloadSeekCost() is called before container.waitForCompletion(),
                    // the returned seek cost is the initial seek cost for scoaPixels.

                    // in scoaPixels, the init columnlet order is: store columnlets of the same column from each
                    // row group back to back.
                    System.out.println("init seek cost:" + scoaPixels.getCurrentWorkloadSeekCost());
                    container.waitForCompletion(1, percentage -> System.out.println(percentage));

                    // naive seek cost is only for test purpose, it is the seek cost of the 'naive'
                    // columnlet order, in which columnlets in each row group are stored back to back following
                    // the column order generated by scoa, e.i. just put row groups back to back.
                    System.out.println("naive seek cost:" + scoaPixels.getOriginSeekCost());

                    // columnlet reordered seek cost is the the seek cost of the columnlet which is reordered
                    // by scoaPixels. The reordering starts from the init columnlet order in which columnlets
                    // of the same column are stored together.
                    System.out.println("columnlet reordered seek cost:" + scoaPixels.getOrderedSeekCost());

                    // start cached cost is the seek cost plus sequential read cost of the init columnlet order
                    // described above.
                    System.out.println("start cached cost:" + scoaPixels.getStartCachedCost());
                    double optimizedCachedCost = scoaPixels.getOrderedCachedCost();

                    // optimized cached cost is the seek cost plus the sequential read cost of the reordered
                    // columnlet order.
                    System.out.println("optimized cached cost:" + optimizedCachedCost);
